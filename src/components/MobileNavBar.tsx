...
const MobileNavBar = ({ mobileNavRef, isMobileNavOpen }: MobileNavBarTypes) => {
  return (
    <nav
      ref={mobileNavRef}
      className={clsx(
        'absolute top-0 lg:hidden backdrop-blur-lg block w-72 bg-white h-screen overflow-hidden transition-all duration-100',
    ...
    >
      <ul className="flex flex-col gap-2 px-2 py-4">
        {HEADER_HEADINGS.map((heading, index) => (
          <li
            key={index}
            className="z-10 py-2 hover:bg-blueTheme/10 px-2 rounded-md"
          >
